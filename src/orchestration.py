...
def check_all_keywords(file_path: Path, logger, youtube, language=DEFAULT_LANGUAGE):
    """
    ...
    with pd.ExcelWriter(
        ...
        link_col = df_audit.columns.get_loc("link") + 1  # +1 for excel index

        # Appliquer le format hyperlien à toute la colonne
        for i, url in enumerate(
            df_audit["link"], start=2
        ):  # start=2 pour sauter l'en-tête
